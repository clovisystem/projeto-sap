...
<?php
    ...
    if($_SESSION['usuario']==null){//para que a paggina de index nao seja acessada antes do login
		...
		///func_nome=str_replace(" ","_",$func_nome);

        $query=mysqli_query($conexao,"select * FROM funcionarios WHERE func_nome='$func_nome' AND func_senha='$func_senha'");
		//$query=mysqli_fetch_array($query);
        $queryRows=mysqli_num_rows($query);
        //echo $queryRows;
        if($queryRows == 0){
			echo'<script>function redireciona(){location.href="login.php";}</script>';
            echo'<div class="alert alert-danger"><h3>Usuário e senhas incorretos</h3></div>';
            echo'<br>';
            echo'<h4>Você será redirecionado ao login em breve.</h4>';
            echo'<script>setTimeout("redireciona()",3000);</script>';
        }
        else{
	...
	<h2 id="titulo"></h2>
		<div class="container">
			<div class="row">
				...
				<br><br><br><br>
				<button class="btn btn-default">Fluxo de Caixa</button>
